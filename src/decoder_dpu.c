...
__host uint32_t init_cycles = 0;
__host uint32_t dequantization_cycles = 0;
__host uint32_t idct_cycles = 0;
__host uint32_t color_space_conversion_cycles = 0;
...
void idct_component_unoptimized(int tasklet_ID, int block_position, int component_ID);
...
void convert_colorspace_component_unoptimized(int tasklet_ID, int cbdr_index, int y_index, int v, int h);
...
int main(){
    ...
    if(tasklet_ID == 0){
        ...
        load_data();
        init_cycles = perfcounter_get();
        accumulated_cycles += init_cycles;
    ...
    dequantize(me());
    if(tasklet_ID == 0) {
        dequantization_cycles = perfcounter_get() - accumulated_cycles;
        accumulated_cycles += dequantization_cycles;
    }
    
    idct(me());
    if(tasklet_ID == 0){ 
        idct_cycles = perfcounter_get() - accumulated_cycles;
        accumulated_cycles += idct_cycles;
    }

    convert_colorspace(me());
    if(tasklet_ID == 0) color_space_conversion_cycles = perfcounter_get() - accumulated_cycles;
